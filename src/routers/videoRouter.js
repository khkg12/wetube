...
videoRouter.get("/:id(\\d+)", watch); //비디오 라우터가 /videos/:id의 주소에 들어가면 다음의 함수실행 여기서 videos주소 생략이유는 server의 use함수때문
videoRouter.route("/:id(\\d+)/edit").get(getEdit).post(postEdit); // 똑같은 url을 사용할 때 편리한 코드방식
